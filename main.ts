...
exports.chooseFile=function(id:string,title:string,buttonLabel:string="Select"){
  dialog.showOpenDialog(
    mainWindow,
    {
      title:title,
    ...
    (filePaths:string[])=>{
      if(filePaths!=undefined) if(filePaths.length>0){
        mainWindow.webContents.send(id, filePaths)
      }
    }
  )
}

let procs:{[id:string]:any}=[]

exports.spawnProc=function (id:string,path:string,args:string[]){    
  try{
    procs[id].kill()
  }catch(err){
    console.log(err)
  ...
  try{            
    ...
    proc.on("error",(err:any)=>{
      console.log(err)
      mainWindow.webContents.send(id, {spawn:false,error:true,data:null,note:"Spawn process error."})
    })
    proc.stdout.on("data",(data:any)=>{
      let lines=data.toString().split("\n").filter((line:any)=>line!="")
      lines.map((line:any)=>mainWindow.webContents.send(id, {spawn:true,error:false,data:line,note:"stdout"}))
    })
    proc.stderr.on("data",(data:any)=>{
      mainWindow.webContents.send(id, {spawn:true,error:true,data:data,note:"stderr"})
    })
    procs[id]=proc
  }catch(err){    
    console.log(err)
    mainWindow.webContents.send(id, {spawn:false,error:true,data:null,note:"Spawn startup error."})
  }
}

exports.writeToProc=function(id:string,content:string){  
  try{
    let proc=procs[id]    
    proc.stdin.write(content+"\n")
  }catch(err){
    console.log(err)
    mainWindow.webContents.send(id, {spawn:true,error:true,data:null,note:"Process write error."})
  }
}

exports.gitApiRequest=function(arg:any){
  ///////////////////////////////////////
  /*
  arg{
    action:string,
  ...
  */
  ...
  let req_url=`${API_URL}/${arg.url}`          
  let action=arg.action
  let id=arg.id
  let config=arg.config  
  config.url=req_url      
  console.log(id,req_url,config)
