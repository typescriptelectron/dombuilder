...
function SCALE_FACTOR() {
    ...
    return rootFontSize == null ? 1 : parseFloat(rootFontSize.replace("px", ""));
}
...
(function (Electron) {
    const fs = require("fs");
    class FileSystemDomStoreDriver {
        fileName(id) { return id + ".json"; }
        store(id, content) {
    ...
    Electron.FileSystemDomStoreDriver = FileSystemDomStoreDriver;
})(Electron || (Electron = {}));
var ElectronGlobal;
(function (ElectronGlobal) {
    ElectronGlobal.electron = require('electron');
    ElectronGlobal.remote = ElectronGlobal.electron.remote;
    ElectronGlobal.mainProcess = ElectronGlobal.remote.require('./main');
    ElectronGlobal.ipcRenderer = ElectronGlobal.electron.ipcRenderer;
})(ElectronGlobal || (ElectronGlobal = {}));
var Electron;
(function (Electron) {
    class FileChooser extends DomElement {
        constructor(id) {
            ...
            this.id = id;
            this.fromStored;
            ElectronGlobal.ipcRenderer.on(this.id, (event, data) => {
                let path = data[0];
                this.path = path;
                this.store;
    ...
    Electron.FileChooser = FileChooser;
})(Electron || (Electron = {}));
...
var Electron;
(function (Electron) {
    class GitBrowser extends DomElement {
        constructor(id) {
            super("div");
            this.tokensById = {};
            this.resetRepo = null;
            this.latestCreateRepoName = "";
            this.id = id;
            this.fs(FONT_SIZE);
            this.fromStored;
            ElectronGlobal.ipcRenderer.on(this.id, (event, data) => {
                ...
                if (data.action == "listdeltokens") {
                    if (data.ok) {
                        ...
                        this.tokencombo.clear().addOptions(tokens.map((token) => new ComboOption(token.id, `${token.id} ${token.note} ${token.scopes.join(",")} ${this.tokensById[token.id]}`))).selectByIndex(0).build();
                        if (this.resetRepo != null) {
                            this.deleteRepo();
                        }
                    }
        ...
        gitApiRequest(action, url, method = "get", data = null) {
            this.scan();
        ...
        addUserClicked(e) {
            this.adduserwindow = new TextInputWindow(this.id + "_addgituserwindow").
    ...
    Electron.GitBrowser = GitBrowser;
})(Electron || (Electron = {}));
DEBUG = false;
//localStorage.clear()
function resetApp() {
